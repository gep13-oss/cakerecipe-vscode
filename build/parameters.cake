...
public class BuildParameters
{
    ...
    public string GitterMessage
    {
        get
        {
            return "@/all Version " + Version.SemVersion + " of the Cake.Recipe VSCode Extension has just been released, https://marketplace.visualstudio.com/items?itemName=gep13.cakerecipe-vscode.  Full release notes: https://github.com/gep13/cakereceipe-vscode/releases/tag/" + Version.SemVersion;
        }
    }
    ...
    public string TwitterMessage
    {
        get
        {
            return "Version " + Version.SemVersion + " of the Cake.Recipe VSCode Extension has just been released, https://marketplace.visualstudio.com/items?itemName=gep13.cakerecipe-vscode. @cakebuildnet @cakecontrib @code  Full release notes: https://github.com/gep13/cakerecipe-vscode/releases/tag/" + Version.SemVersion;
        }
    }
        ...
        )
    {
        ...
        return new BuildParameters {
            ...
            IsMasterRepo = StringComparer.OrdinalIgnoreCase.Equals("gep13/cakerecipe-vscode", buildSystem.AppVeyor.Environment.Repository.Name),
            ...
            Chocolatey = new ChocolateyCredentials (
                apiKey: context.EnvironmentVariable("CAKERECIPEVSCODE_CHOCOLATEY_APIKEY"),
                sourceUrl: context.EnvironmentVariable("CAKERECIPEVSCODE_CHOCOLATEY_SOURCEURL") ?? "https://push.chocolatey.org"
            ),
            GitHub = new BuildCredentials (
                userName: context.EnvironmentVariable("CAKERECIPEVSCODE_GITHUB_USERNAME"),
                password: context.EnvironmentVariable("CAKERECIPEVSCODE_GITHUB_PASSWORD")
            ),
            Marketplace = new VisualStudioMarketplaceCredentials (
                token: context.EnvironmentVariable("CAKERECIPEVSCODE_VSMARKETPLACE_TOKEN")
            ),
            Wyam = new WyamCredentials (
                accessToken: context.EnvironmentVariable("CAKERECIPEVSCODE_WYAM_ACCESS_TOKEN"),
                deployRemote: context.EnvironmentVariable("CAKERECIPEVSCODE_WYAM_DEPLOY_REMOTE"),
                deployBranch: context.EnvironmentVariable("CAKERECIPEVSCODE_WYAM_DEPLOY_BRANCH")
            ),
            Gitter = new GitterCredentials (
                token: context.EnvironmentVariable("CAKERECIPEVSCODE_GITTER_TOKEN"),
                roomId: context.EnvironmentVariable("CAKERECIPEVSCODE_GITTER_ROOM_ID")
            ),
            Twitter = new TwitterCredentials (
                consumerKey: context.EnvironmentVariable("CAKERECIPEVSCODE_TWITTER_CONSUMER_KEY"),
                consumerSecret: context.EnvironmentVariable("CAKERECIPEVSCODE_TWITTER_CONSUMER_SECRET"),
                accessToken: context.EnvironmentVariable("CAKERECIPEVSCODE_TWITTER_ACCESS_TOKEN"),
                accessTokenSecret: context.EnvironmentVariable("CAKERECIPEVSCODE_TWITTER_ACCESS_TOKEN_SECRET")
            ),
            IsPublishBuild = new [] {
                "ReleaseNotes",
            ...
            WebLinkRoot = "cakerecipe-vscode",
            WebBaseEditUrl = "https://github.com/gep13/cakerecipe-vscode/tree/develop/input/"
        };
    }
}
