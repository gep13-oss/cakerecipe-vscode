...
Setup(context =>
{
    ...
    Information("Building version {0} of cakerecipe-vscode ({1}, {2}) using version {3} of Cake. (IsTagged: {4})",
...
Task("Create-Release-Notes")
    .Does(() =>
{
    GitReleaseManagerCreate(parameters.GitHub.UserName, parameters.GitHub.Password, "gep13", "cakerecipe-vscode", new GitReleaseManagerCreateSettings {
    ...
    .Does(() =>
{
    ...
    var packageFile = File("cakerecipe-vscode-" + parameters.Version.SemVersion + ".vsix");
    ...
    .Does(() =>
{
    ...
    var nuspecFile = File("./chocolatey/cakerecipe-vscode.nuspec");

    EnsureDirectoryExists(parameters.ChocolateyPackages);
    var extensionFile = MakeAbsolute((FilePath)("./BuildArtifacts/cakerecipe-vscode-" + parameters.Version.SemVersion + ".vsix"));
    ...
    files.Add(new ChocolateyNuSpecContent { Source = extensionFile.ToString(), Target = "tools/cakerecipe-vscode.vsix" });
...
.Does(() =>
{
    var buildResultDir = Directory("./BuildArtifacts");
    var packageFile = File("cakerecipe-vscode-" + parameters.Version.SemVersion + ".vsix");
    var chocolateyPackageFile = File("_Packages/chocolatey/cakerecipe-vscode." + parameters.Version.SemVersion + ".nupkg");
    ...
    .Does(() =>
{
    ...
    {
        GitReleaseManagerAddAssets(parameters.GitHub.UserName, parameters.GitHub.Password, "gep13", "cakerecipe-vscode", parameters.Version.Milestone, package.ToString());
    }

    GitReleaseManagerClose(parameters.GitHub.UserName, parameters.GitHub.Password, "gep13", "cakerecipe-vscode", parameters.Version.Milestone);
})
.OnError(exception =>
{
    ...
    .Finally(() =>
{
    if(publishingError)
    {
        throw new Exception("An error occurred during the publishing of cakerecipe-vscode.  All publishing tasks have been attempted.");
    }
});
